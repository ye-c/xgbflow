...
def classifier_verify(model, x_train, y_train, x_test, y_test, x_verify, y_verify, draw_out=False, title='title'):
    ...
    for name, data, label in [
        ...
        if name == '验证集':
            top_data = top_status(label, y_pred)
...
def calc_feature_dist(train_df, verify_df, tar_fea, val_ls, cutn=10):
    cols = ['label', tar_fea]
