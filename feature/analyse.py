...
# def ratio(dfls=[], keys=None, label='label'):
#     if not isinstance(dfls, list):
#         dfls = [dfls]
#     keys = ['d%s' % i for i in range(len(dfls))] if not keys else keys
#     res = []
...
def ratio(df_dict, label='label'):
    assert isinstance(df_dict, dict), 'Not dict'
    res = []
